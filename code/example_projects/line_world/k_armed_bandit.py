...
def k_armed_bandit(fig_dir_path: Optional[Path]):
    ...
    if fig_dir_path:
        ...
        tikzplotlib.save(fig_dir_path / "10_armed_bandit.tex", extra_axis_parameters=[
            'width=0.9\\textwidth',
            'height=0.3\\textwidth'
        ])
    else:
...
def k_bandit_violin(fig_dir_path: Optional[Path]):
    ...
    if fig_dir_path:
        tikzplotlib.clean_figure()
        tikzplotlib.save(fig_dir_path / "k_armed_bandit_violin.tex", extra_axis_parameters=[
            'width=0.9\\textwidth',
            'height=0.3\\textwidth',
            r'xticklabel={%' '\n'
            r'    \pgfmathtruncatemacro{\IntegerTick}{\tick}%' '\n'
            r'    \pgfmathprintnumberto[verbatim,fixed,precision=3]{\tick}\tickAdjusted%' '\n'
            r'    \pgfmathparse{\IntegerTick == \tickAdjusted ? 1: 0}%' '\n'
            r'    \ifnum\pgfmathresult>0\relax$\IntegerTick$\else\fi%' '\n'
            r'}'
        ])
    else:
        plt.show()
...
if __name__ == '__main__':
    fig_dir_path = Path(__file__).parent.parent.parent.parent / "reports" / "preliminary_thesis" / "figures"
