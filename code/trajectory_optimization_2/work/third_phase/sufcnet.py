...
class SUFullyConnectedNetwork(TFModelV2):
    """Generic fully connected network implemented in ModelV2 API."""

    def __init__(self, obs_space: gym_spaces.Space,
                 action_space: gym_spaces.Space, num_outputs: int,
                 model_config: ModelConfigDict, name: str,
                 dropouts=None, training=True):
        super(SUFullyConnectedNetwork, self).__init__(
        ...
        for size, dropout_rate in zip(hiddens[:-1], dropouts[:-1]):
            ...
            if dropout_rate:
                last_layer = tf.keras.layers.Dropout(
                    dropout_rate,
                    name="fc_dropout_{}".format(i))(last_layer)
            i += 1
    ...
    def forward(self, input_dict: dict[str, TensorType],
        ...
        model_out, self._value_out = self.base_model(input_dict["obs_flat"], training=self.training)
