...
The policy is a mapping from the state of the environment to the probabilities of the agent selecting each action. If the policy at time $t$ is $\pi$, then $\pi(a|s)$ is probability of selecting action $a$ if the current state is $s$.
...
\begin{equation}
  \mathbb{E}_\pi \left [ \left. G_t \right| S_t=s \right ] = \mathbb{E}_\pi \left [ \left. \sum_{k=t+1}^{T}\gamma^{k-t-1}R_{k} \right| S_t=s \right ], \textup{ for all } s \textup{ in } \mathcal{S}
\end{equation}
...
\begin{equation}
  \mathbb{E}_\pi \left [ \left. G_t \right| S_t=s, A_t=a \right ] = \mathbb{E}_\pi \left [ \left. \sum_{k=t+1}^{T}\gamma^{k-t-1}R_{k} \right| S_t=s, A_t=a  \right ], \textup{ for all } s \textup{ in } \mathcal{S}
\end{equation}
