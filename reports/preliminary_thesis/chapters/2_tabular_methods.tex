...
In order to use TD prediction for the control problem the pattern of \acrfull{gpi} can be used, only this time using TD methods for the evaluation or prediction part. Approaches fall under two classes, on-policy and off-policy. This section focuses on an on-policy \gld{td} control method.
...
Another option is Q-learning, an off-policy TD control method. In this case the learned action-value. $Q$, directly approximates $q_*$, the optimal action-value function, independent of the policy being followed. The Q-learning update rule is defined by

\begin{equation}
  Q(S_t, A_t) \leftarrow Q(S_t, A_t) + \alpha \left [  R_{t+1} + \gamma\ \underset{a}{max}\ Q(S_{t+1}, a) - Q(S_t, A_t) \right ]
\end{equation}}

The advantage of this method is that it drastically simplifies the analysis of the algorithm and enables early convergence proofs. The policy still has an effect since it determines which states-action pairs are visited and updated, however the only thing that is required is that all pairs continue to be updated.

The disadvantage is that since the action-value function learned is independent of the policy, the agent doesn't take into account the possibility of it taking a non-optimal action, which in turn may lead to a solution worse than that obtained by a on-policy method.