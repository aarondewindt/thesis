...
The optimal policies can easily be found once the optimal value functions $v_*$ or $q_*$ which satisfy the Bellman optimality equations shown below are known.
...
\begin{figure}[ht]			
	\centering
	\input{figures/on_off_policy.tex}	
	\caption{Grid world with the safe and optimal paths learned by on-and off-policy methods, where the hashed area is the cliff.}
...
\begin{figure}[ht]			
	\centering
	\input{figures/on_off_policy_results.tex}			
	\caption{Example reward distribution for a 5-armed bandit problem.}
	\label{fig:on_off_policy_results}
\end{figure}


\subsection{$n$-step Bootstrapping}
Consider the expected return used with Monte Carlo methods shown below. It's based on the rewards starting from time $t$ until the end of the episode. Thus while the update does include information throughout a long period of time, it's only possible to update the values after the episode has ended. 

\begin{equation}
  G_t \doteq R_{t+1} + \gamma R_{t+2} + \gamma^2 R_{t+3} + \ldots + \gamma^{T-t-1}R_T
\end{equation}

In contrast the expected return for one-step TD only uses single reward after one time-step. For many applications it's desirable to update as often as possible, but bootstrapping works best over a period of time after significant and recognizable change has occurred. With one-step TD methods these two lengths of time are the same, so a compromise must be made.

\begin{equation}
  G_{t:t+1} \doteq R_{t+1} + \gamma V_t(S_{t+1})
\end{equation}

While both of these methods are effective in solving \gls{mdp} problems, the best solutions are usually somewhere in between. $n$-step Bootstrapping methods provide a spectrum of methods with Monte Carlo in one end and one-step TD on the other by enabling bootstrapping to occur over multiple time steps.

An example family of $n$-step Bootstrapping methods are $n$-step TD Prediction methods. For these methods the \emph{$n$-step return} is as follows,

\begin{equation}
  G_{t:t+n} \doteq R_{t+1} + \gamma R_{t+2} + \ldots + \gamma^{n-1} R_{t+n} + \gamma^n V_{t+n-1}(S_{t+n})
  \label{eq:nstepbootreturn}
\end{equation}

As it is clear from the Equation~\ref{eq:nstepbootreturn}, the expected return uses the reward of multiple steps, thus allowing the value function to be updated on each time-step based on information collected over a longer period of time.
