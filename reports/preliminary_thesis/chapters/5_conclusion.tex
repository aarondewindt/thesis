\chapter{Conclusion}
The fist set of research questions focus on the methods commonly used for computer orbital ascent trajectories. 
...
The most common methods used to compute orbital ascent trajectories are based on trajectory optimization, a set of mathematical tools used to compute open-loop solutions to optimal control problems. The reason is because these methods are well suited for solving problems that are non-linear, high dimensional and sensitive to errors, a set of traits which unfortunately true when computing orbital ascent trajectories.
...
Furthermore during the derivation of the surrogate problem a new set of adjoint variables are added, which also need to be initialized and optimized together the controls and states, and since these adjoint variables don't have a physical representation, it may be difficult to find appropriate initial values when solving the problem.
...
The main project for this thesis is the use of \acrshort{rl} to compute closed-loop solutions for orbital ascent trajectories. Although the overall training process is likely more computationally intensive than computing a single solution using trajectory optimization, a already trained agent would be able to compute the optimal or close to optimal solution with just one simulation run.

\acrshort{rl} can be a cost effective solution when a large set of optimal trajectories are required, such as during preliminary design of a new launch vehicle or as part of a larger \acrfull{mdo} project. So far most research questions have been answered in this preliminary thesis. The remaining questions that still need to be investigated are:
...
One of the goals of the project is to check how many times the trained agent will need to be used to make it cost effective over using the \acrfull{to} based program for every trajectory. To do this a controlled environnement is needed to run the training and optimizations. In order to create and control these computation environments the code will be developed to run inside Docker containers. These containers can then be deployed into dedicated cloud computing instances for consistent access to computation resources. All logged computation times should then form a reliable measure of computational requirements.

To assess the accuracy of the \acrshort{rl} based program, the results of it can be compared to the results yielded by the \acrshort{to} based program. If accurate both should yield trajectories with the same or similar performance, with the performance in this case being measured by propellant utilization.

If the \acrshort{rl} based program is not able to achieve the accuracy of the \acrshort{to} based program, then another option is to combine \acrshort{rl} methods and \acrshort{to} methods to produce a hybrid program by using the trained \acrshort{rl} agent to generate initial conditions for the \acrshort{to} method.
